	...
	public static Class<? extends ChatMember> getChatMemberClass(JsonObject jsonObject) {
		...
		if (element == null) {
			throw new JsonNotFoundKey(
					String.format("Not found the key \"%s\" in %s.", CHAT_MEMBER_KEY, ChatMember.class.getName()));
		}
		...
		switch (value) {
			...
			default -> throw new JsonTypeNotDefined(
					String.format("Given \"%s\" in %s is not defined.", CHAT_MEMBER_KEY, ChatMember.class.getName()));
		}
		return clazz;
	}
}
