...
public class DeserializerProxy {
	...
	public static Class<? extends InlineQueryResult> getInlineQueryResultClass(JsonObject jsonObject) {

		JsonElement element = jsonObject.get(INLINE_QUERY_RESULT_KEY);
		if (element == null) {
			throw new JsonNotFoundKey(String.format("Not found the key \"%s\" in %s.", INLINE_QUERY_RESULT_KEY,
					InlineQueryResult.class.getName()));
		}
		...
		switch (value) {
			...
			case InlineQueryResultAudio.TYPE -> {
				clazz = InlineQueryResultCachedAudio.isCached(jsonObject) ? InlineQueryResultCachedAudio.class
						: InlineQueryResultAudio.class;
			}
			case InlineQueryResultVideo.TYPE -> {
				clazz = InlineQueryResultCachedVideo.isCached(jsonObject) ? InlineQueryResultCachedVideo.class
						: InlineQueryResultVideo.class;
			}
			case InlineQueryResultDocument.TYPE -> {
				clazz = InlineQueryResultCachedDocument.isCached(jsonObject) ? InlineQueryResultCachedDocument.class
						: InlineQueryResultDocument.class;
			}
			case InlineQueryResultGif.TYPE -> {
				clazz = InlineQueryResultCachedGif.isCached(jsonObject) ? InlineQueryResultCachedGif.class
						: InlineQueryResultGif.class;
			}
			case InlineQueryResultMpeg4Gif.TYPE -> {
				clazz = InlineQueryResultCachedMpeg4Gif.isCached(jsonObject) ? InlineQueryResultCachedMpeg4Gif.class
						: InlineQueryResultMpeg4Gif.class;
			}
			case InlineQueryResultVoice.TYPE -> {
				clazz = InlineQueryResultCachedVoice.isCached(jsonObject) ? InlineQueryResultCachedVoice.class
						: InlineQueryResultVoice.class;
			}
			case InlineQueryResultPhoto.TYPE -> {
				clazz = InlineQueryResultCachedPhoto.isCached(jsonObject) ? InlineQueryResultCachedPhoto.class
