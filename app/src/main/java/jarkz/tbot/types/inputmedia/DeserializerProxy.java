	...
	public static Class<? extends InputMedia> getInputMediaClass(JsonObject jsonObject) {
		...
		if (element == null) {
			throw new JsonNotFoundKey(
					String.format("Not found the key \"%s\" in %s.", INPUT_MEDIA_KEY, InputMedia.class.getName()));
		}
		...
		switch (value) {
			...
			default -> throw new JsonTypeNotDefined(
					String.format("Given \"%s\" in %s is not defined.", INPUT_MEDIA_KEY, InputMedia.class.getName()));
		}
		return clazz;
	}
}
